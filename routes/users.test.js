...
describe('GET /users/:username', function() {
	...
	test('works for admins', async function() {
		const resp = await request(app).get(`/users/u1`).set('authorization', `Bearer ${a1Token}`);
		expect(resp.body).toEqual({
			user : {
				username  : 'u1',
				firstName : 'U1F',
	...
	test('admin request: not found if user not found', async function() {
		const resp = await request(app).get(`/users/nope`).set('authorization', `Bearer ${a1Token}`);
		expect(resp.statusCode).toEqual(404);
	});

	test('user request: unauth if user not found', async function() {
...
describe('PATCH /users/:username', () => {
	...
	test('admin request: not found if no such user', async function() {
		const resp = await request(app)
			.patch(`/users/nope`)
			.send({
				firstName : 'Nope'
	...
	test('user request: unauthorized if no such user', async function() {
		const resp = await request(app)
			.patch(`/users/nope`)
			.send({
				firstName : 'Nope'
...
describe('DELETE /users/:username', function() {
	...
	test('admin request: not found if user missing', async function() {
		const resp = await request(app).delete(`/users/nope`).set('authorization', `Bearer ${a1Token}`);
		expect(resp.statusCode).toEqual(404);
	});
