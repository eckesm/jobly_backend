...
describe('POST /companies', function() {
	...
	test('ok for admins', async function() {
		const resp = await request(app).post('/companies').send(newCompany).set('authorization', `Bearer ${a1Token}`);
...
describe('GET /companies', function() {
	...
	test('filtered minEmployees', async function() {
	...
	test('filtered maxEmployees', async function() {
	...
	test('filtered name', async function() {
...
describe('DELETE /companies/:handle', function() {
	test('works for admins', async function() {
		const resp = await request(app).delete(`/companies/c1`).set('authorization', `Bearer ${a1Token}`);
  ...
  test('fails for users not admins', async function() {
	...
	test('not found for no such company', async function() {
		const resp = await request(app).delete(`/companies/nope`).set('authorization', `Bearer ${a1Token}`);
		expect(resp.statusCode).toEqual(404);
	});
});
